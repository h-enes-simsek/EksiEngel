...
def _is_version_4_or_later(version):
...
class WriteActionViewSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        if _is_version_4_or_later(attrs.get('version')):
            missing_fields = {
                field_name: 'This field is required for extension version 4.0 and newer.'
                for field_name in ('job_id', 'job_duration')
