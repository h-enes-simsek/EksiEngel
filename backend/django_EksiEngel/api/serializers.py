...
class WriteActionConfigSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        exclude = ('id','action')
        extra_kwargs = {
            'eksi_sozluk_url': {'max_length': 500},
        }
...
class WriteEksiEngelUserSerializer(serializers.ModelSerializer):
    eksisozluk_name = serializers.CharField(max_length=100)
...
class WriteActionViewSerializer(serializers.ModelSerializer):
    ...
    def validate_author_list(self, value):
        """Limit author list to prevent DoS attacks"""
        if len(value) > 10000:
            raise serializers.ValidationError("Yazar listesi çok büyük (maksimum 10000)")
        return value
        
    def validate_log(self, value):
        """Limit log field size"""
        if value and len(value) > 10000:
            raise serializers.ValidationError("Log alanı çok büyük (maksimum 10000 karakter)")
        return value
